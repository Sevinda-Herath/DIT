...
.panel:before{content:"";position:absolute;inset:0;background:radial-gradient(circle at 82% 18%,rgba(140,75,255,.25),transparent 65%),radial-gradient(circle at 14% 86%,rgba(60,160,255,.18),transparent 70%);mix-blend-mode:screen;pointer-events:none;}
...
.table {width:100%;border-collapse:separate;border-spacing:0;font-size:1.25rem;min-width:820px;}
.table thead th {background:linear-gradient(135deg,rgba(140,75,255,0.4),rgba(80,40,150,0.45));color:#fff;font-weight:600;text-transform:uppercase;font-size:1.1rem;letter-spacing:.5px;position:sticky;top:0;backdrop-filter:blur(3px);padding:12px 14px;border-bottom:1px solid rgba(255,255,255,0.08);} 
...
<main class="dashboard container">
  <div class="status-bar" style="padding:10px 18px;border-radius:14px;font-size:1.25rem;font-weight:500;display:flex;flex-wrap:wrap;gap:12px;align-items:center;background:linear-gradient(100deg,rgba(140,75,255,0.35),rgba(40,30,60,0.65));border:1px solid rgba(140,75,255,0.45);box-shadow:0 8px 26px -10px rgba(140,75,255,0.55) inset,0 2px 8px -2px rgba(0,0,0,.7);margin-bottom:6px;">
    Logged in as <strong><?= h($user['username']) ?></strong> (role: <strong><?= h($user['role']) ?></strong>) | Visible users: <?= count(array_filter($pdoUsers, fn($r)=>in_array($r['role'], ['user','organizer']))) ?>
  </div>
